...
\begin{tikzpicture}[node distance=1.6cm and 2cm, auto, >=latex', thick]
    ...
    \node[draw, rectangle, rounded corners, minimum width=4cm, minimum height=1cm, align=center, below=of pi] (arduino) {Arduino\\(2-Way Communication)};
...
\draw[arrow] (continue.south) 
    -- ++(0,-0.8)
    -| ([xshift=-2.5cm]nav.west)
    |- (nav.north);
...
\subsubsection{Pill Dispensing Subsystem}
...
\node (obstacle) [decision, below of=pid, yshift=-0.5cm] {Zone Detected?};
...
\input{chapters/software_implementation}

\section{Face Recognition System}
\input{chapters/face_recognition}

\section{System Integration}
\input{chapters/system_integration}
